...
Для обучения через командную строку используйте утилиту `train` с доступными параметрами:

```bash
./train --input учебник.txt --output модель.pt [опции]
```

Доступные опции для командной строки:
```
...
│   │   ├── inference.h
│   │   ├── inference.cpp
│   │   └── train_main.cpp   # Утилита командной строки для обучения
